...
const DEFAULT_ROM_URL = 'index/src/game.gba';

function resolveAutoBootRomUrl() {
    try {
        if (typeof window === 'undefined' || typeof window.location === 'undefined') {
            return DEFAULT_ROM_URL;
        }

        var currentUrl = new URL(window.location.href);
        var romParam = currentUrl.searchParams.get('rom');

        if (!romParam) {
            return DEFAULT_ROM_URL;
        }

        var romString = romParam.trim();
        if (!romString) {
            return DEFAULT_ROM_URL;
        }

        var resolvedRom = new URL(romString, currentUrl.href);
        if (resolvedRom.origin !== currentUrl.origin) {
            console.warn('Ignoring cross-origin ROM URL:', romString);
            return DEFAULT_ROM_URL;
        }

        return resolvedRom.pathname + resolvedRom.search + resolvedRom.hash;
    } catch (err) {
        console.warn('Failed to resolve ROM URL from query parameter.', err);
        return DEFAULT_ROM_URL;
    }
}

const AUTO_BOOT_ROM_URL = resolveAutoBootRomUrl();
let autoBootTriggered = false
...
function autoBootRom() {
    if (autoBootTriggered || !AUTO_BOOT_ROM_URL) {
        return
    }
    if (!isWasmReady) {
        return
    ...
    var selectRom = document.getElementById('select-rom')
    var chooseBtn = document.getElementById('btn-choose')
    if (chooseBtn) {
        if (!chooseBtn.dataset.defaultLabel) {
            chooseBtn.dataset.defaultLabel = chooseBtn.textContent
        }
        chooseBtn.disabled = true
        chooseBtn.textContent = 'Loading default game...'
    }
    if (selectRom) {
        selectRom.hidden = false
    }
    tryInitSound()
    fetch(AUTO_BOOT_ROM_URL).then(function (resp) {
        if (!resp.ok) {
            throw new Error('HTTP ' + resp.status)
        }
        return resp.arrayBuffer()
    }).then(function (ab) {
        romFileName = AUTO_BOOT_ROM_URL.split('/').pop() || 'game.gba'
        loadRomArrayBuffer(ab)
    }).catch(function (err) {
        console.error('Auto boot failed', err)
        if (chooseBtn) {
            chooseBtn.disabled = false
            chooseBtn.textContent = chooseBtn.dataset.defaultLabel || 'Choose File...'
        }
        showMsg('Failed to load bundled game.')
    })
}
...
function loadRomArrayBuffer(arrayBuffer) {
    ...
    loadSaveGame(0, function () {
        Module._emuResetCpu()
        var cheatsApplied = applyCheatCode()
        if (cheatsApplied) {
